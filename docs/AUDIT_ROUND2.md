...
**STATUS: CONFIRMED AND FIXED, in two parts.** The fabrication half was closed in an earlier round: the empty-pool branch now returns `hit_rate: nSignal === 0 ? null : hits / nSignal` and the mirror for the false-alarm rate, so the rate that IS computable is reported and only the absent one is null; the types are `number | null` and the codebook says EMPTY rather than 0 for both. The second half of the finding was still open and is fixed here — `d_prime_estimable` was computed on every summary, stored in the record, and then dropped from `09_rt_summary.csv`, so an empty `d_prime` cell meant either 'not estimable' or 'not written' with nothing in the file to separate them. `d_prime_unstable` cannot stand in: with 20 signal and 12 noise trials the smallest achievable SE is about 0.46, so that flag is true for every block the study can produce. The column is now exported and documented, and `analysis_long.csv`'s codebook entry for `d_prime` no longer claims the only reason it can be empty is an unfinished block.
...
**STATUS: PARTLY CONFIRMED — one premise is WRONG, the rest is real but UNMEASURABLE from this repository, and it is left open deliberately.**

Verified: `theme.css`:53 applies `transform: scale(var(--vl-scale))`; `viewportScale.ts`:56 and :102 cap the scale at 1 and quantise it to multiples of 0.02, so a scaled device renders glyphs at fractional pixel sizes — 22 x 0.86 = 18.92 px, 22 x 0.76 = 16.72 px, 22 x 0.50 = 11.00 px. There is no `-webkit-font-smoothing`, `text-rendering` or `will-change` anywhere in `theme.css`, so nothing overrides the browser's default choice.

**The finding's central premise is wrong, though.** It assumed unscaled devices take a different rasterisation path. They do not: `transform: scale(1)` computes to `matrix(1, 0, 0, 1, 0, 0)`, not `none`, so EVERY device renders `#root` through a transform — and `#root` additionally carries `isolation: isolate` (`theme.css`:65), which forces a stacking context regardless of the scale. Whatever antialiasing regime the browser selects for a transformed layer, it selects on all devices alike. What genuinely differs by device is the sub-pixel glyph metrics, not the presence of a transform.

**NOT ESTABLISHED, and not establishable from here:** whether the browser switches from subpixel to grayscale antialiasing between `scale(1)` and `scale(0.86)`, and whether any resulting stroke-weight change interacts with the ink colour or the polarity. That needs a photometric or screenshot comparison of two physical tablets rendering the same condition. The original finding conceded it did not measure this, and neither has anything since.

**Left OPEN rather than closed.** It sits in the same causal family as the aspect-ratio finding — device-dependent presentation with no device factor in the design — but unlike that one, nothing here is computable from the code, so it cannot honestly be reported as a known bias. It is a measurement that has not been taken. If it matters, the same one-session device pilot that settles the camera frame rate can photograph the same passage on both tablets and settle this too.
...
**STATUS: CONFIRMED AND FIXED.** Verified: `CAMERA_SETUP` renders `CameraDeclined` whenever `media_consent.camera_metrics !== true`, so `tracking.start()` is never called, `mediaSource()` is null and `captureMedia` returns silently. A participant who declines blink measurement but agrees to be photographed has ticked a box the app can never act on. Fixed WITHOUT a new column, because the state is already fully determined by two consent flags that both ship: `auditBundle` now raises `grant_not_exercisable` when `setup_photos` is granted and `camera_metrics` is not, and it reaches the dataset through `16_integrity_report.csv`. Severity is warning rather than error — no rule was broken and no value is wrong; the absence was merely unreadable, which is a different failure. Failing closed stays the behaviour; failing closed SILENTLY was the defect.
