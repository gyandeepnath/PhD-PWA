...
| Visual search | `search_time_ms` | LMM, **censored** | `termination_mode` says whether the block ended by completion or by the 40 s cap (this table named `search_termination`, which the export does not write — the same defect class that stopped the R template running). Capped rows are a lower bound; treating them as measurements biases the mean downward. Either model them as censored or report the completion rate alongside. |
