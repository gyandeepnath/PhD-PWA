...
Phase III — Session 1: 10 display conditions under illumination level A, in Williams order
Phase IV — Session 2 after 48–72 h: the same 10 conditions under illumination level B
...
The second illumination block advances the Williams row by one, so a participant does not meet the conditions in the same serial order twice. Reusing the row would correlate serial position perfectly with condition within a participant across both sessions, letting a position effect masquerade as a condition effect in the same direction twice; advancing it preserves complete balance across each block of ten, since every row is still used once.

Illumination order is counterbalanced orthogonally to that row, which requires care: assigning dim-first to odd enrolment numbers would not be orthogonal, because the row is the enrolment index modulo ten and two divides ten, so the row would determine the order entirely. Order is therefore alternated by parity and inverted after each completed block of ten. This splits dim-first and moderate-first five to five within every block, so order is confounded with neither the condition row nor recruitment date, and pairs each row with both orders once across twenty participants — balance completing at n = 20, which divides 130.
...
The manipulation cannot be blinded, since polarity and colour are visible, so expectation effects on subjective ratings cannot be excluded, although objective markers, the absence of feedback and counterbalancing mitigate this. Sessions accumulate generic fatigue and boredom alongside visual fatigue, which are counterbalanced, measured and modelled rather than eliminated, and administering the NASA Task Load Index once per session limits workload inference to the illumination contrast. Because illumination is manipulated between sessions, its effect is more exposed to between-session variance in time of day, sleep and circadian phase, although order is counterbalanced and covariates recorded; and PERCLOS, while serving as a sleepiness covariate, is itself insensitive to moderate drowsiness.
