...
The level sits within the 300–500 lux band specified for work with self-luminous displays, at its lower bound so the display is not washed out. Critically, it is high enough that room light rather than the display dominates the illuminance on the reader's face: because the ocular measures are camera-derived, and the modelled ratio of face illuminance between polarities is about 2.3 at 10 lux against 1.05 at 300, a dimmer room would make camera exposure vary with the primary independent variable.
...
**Validation.** A subsample of twenty participants contributes two 3-minute segments each for masked frame-by-frame annotation, against which the automated classifier is assessed by Cohen's kappa and Bland–Altman bias (criteria: kappa ≥ 0.60, bias within ±5 percentage points).
