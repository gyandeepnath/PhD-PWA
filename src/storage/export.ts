...
export const CODEBOOK: Record<string, string>[] = [
  ...
  { file: '11_participant.csv', column: 'cvd_screen_correct', type: 'integer', unit: 'count', role: 'qc', description: `CONFUSION plates identified correctly on the app's own digital screen, run on the study display. `
    + `The participant is shown ${SCREEN_TEST_PLATES + 1} plates: a greyscale control that everyone should pass, which is a validity `
    + `check and is counted in NEITHER the numerator nor the denominator, and ${SCREEN_TEST_PLATES} red-green confusion plates, which are `
    + `both. So this is out of ${SCREEN_TEST_PLATES}, not out of ${SCREEN_TEST_PLATES + 1}. Missing the control sets cvd_status to screen_inconclusive whatever this `
