...
export const CODEBOOK: Record<string, string>[] = [
  ...
  { file: '02_conditions.csv', column: 'passage_repeat_number', type: 'integer', unit: 'count', role: 'covariate', description: "How many times this participant has read this passage under the PLANNED protocol, counting this run. Constant at 1 under the single-sitting design, where ten passages cover ten condition-runs; above 1 only in earlier two-sitting data. It is derived from the illumination block, so it does NOT count an unplanned re-reading: use attempt_number for that." },
  ...
  { file: '01_session_info.csv', column: 'calibration_ear_samples', type: 'integer', unit: 'count', role: 'qc', description: 'Frames of the calibration routine that yielded a usable eye-aspect ratio, and so fed calibration_ear_baseline. A working camera contributes several hundred; a low value means the baseline every blink threshold is a fraction of rests on very little, and the ocular measures for that sitting should be treated with caution. Blank for sittings recorded before this was captured.' },
