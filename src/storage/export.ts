...
export const CODEBOOK: Record<string, string>[] = [
  ...
  { file: '02_conditions.csv', column: 'adaptation_ms_before', type: 'integer', unit: 'ms', role: 'qc', description: "Grey-field adaptation actually delivered before this condition, in milliseconds. Doubled on a polarity switch. The FIRST condition of a sitting now gets a grey field too — it used to be the exception, which meant a negative-polarity run at position 0 began light-adapted from the cream setup UI while a positive-polarity one did not, an asymmetry confounded with the primary factor. So a session_position=0 row carries a normal adaptation value; a 0 here means the field was not delivered, not that it was the first condition. Compare against adaptation_ms_planned." },
  ...
  { file: '02_conditions.csv', column: 'adaptation_ms_planned', type: 'integer', unit: 'ms', role: 'qc', description: "The grey-field duration the protocol called for before this condition — 60,000 ms normally, 120,000 ms across a polarity switch. Compare against adaptation_ms_before, which is what the participant actually saw: they diverge when the device slept or the app was backgrounded during the field, because the countdown is frame-driven and stops. A large gap means the light-adaptation control for that condition was not delivered. Blank for rows written before this was recorded." },
  ...
  { file: '05_visual_search.csv', column: 'distractor_words', type: 'integer', unit: 'count', role: 'meta', description: "Non-target tappable tokens in the passage: the correct-rejection pool behind search_d_prime. Tappable is the task's own definition — any non-whitespace token, so a standalone punctuation mark or a bare numeral is a trial the participant could have committed a false alarm on. That makes this a shade larger than the passage's word count (under 1%), and it is the right pool because it is the set of things that could be tapped. It formerly counted the whitespace runs too, because the renderer's tokeniser keeps them to preserve spacing — roughly doubling the value and shrinking every false-alarm rate accordingly." },
...
export function buildExportFiles(input: SessionBundle): ExportFile[] {
  ...
  csv('01_session_info.csv',
    ['participant_id', 'experiment_date', 'enrolment_number', 'session_index', 'session_status', 'conditions_completed', 'session_complete', 'conditions_per_session', 'condition_offset', 'ambient_lux', 'ambient_illumination_level', 'illumination_block', 'protocol_pass', 'repeat_run_note', 'illumination_order_first', 'lux_start', 'lux_middle', 'lux_end', 'lux_n_readings', 'lux_checkpoints_logged', 'lux_complete', 'lux_mean', 'lux_max_deviation', 'lux_logged_all_in_range', 'lux_deviation_note', 'screen_white_luminance_cd_m2', 'brightness_percent', 'session_duration_min', 'app_version', 'git_hash', 'condition_def_hash', 'schema_version', 'device_type', 'screen_resolution', 'stimulus_scale', 'layout_viewport', 'consent_given', 'consent_camera_metrics', 'consent_setup_photos', 'consent_annotation_video', 'media_items_retained', 'preflight_complete', 'e2e_timing', 'stimulus_font_ok', 'caffeine_today_session', 'hours_since_sleep_session', 'gaze_calibration_valid', 'calibration_ear_baseline', 'calibration_pitch_baseline_frac', 'calibration_targets_detected', 'calibration_ear_samples', 'calibration_runs'],
    [{
