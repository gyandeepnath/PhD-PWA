...
export const ANALYSIS_CODEBOOK: AnalysisColumn[] = [
  ...
  { column: 'gaze_trust', role: 'qc', unit: 'good|thin|unusable', missing: 'no calibration record for the sitting',
    description: 'How much evidence the gaze fit rests on. good = two thirds of the nine targets were tracked through at least half their dwell; thin = it cleared the acceptance bar on very little data; unusable = the fit was rejected. FILTER GAZE MEASURES ON THIS, not on gaze_calibrated: that flag is TRUE for both good and thin, and a thin fit produces thresholds closer to noise than to measurement. Session-level, so it repeats across the sitting rows.' },
  { column: 'gaze_targets_well_covered', role: 'qc', unit: 'targets', missing: 'no calibration record for the sitting',
    description: 'How many of the nine calibration targets were tracked through at least half their dwell, as opposed to merely registering at all. The evidence behind gaze_trust. Session-level, so it repeats across the sitting rows.' },
  ...
  { column: 'condition_interrupted', role: 'qc', unit: 'boolean', missing: 'never empty',
    description: 'TRUE when, somewhere in this condition, the app was backgrounded or the tablet was held in portrait for more than 2 s. Both stop the participant responding while the timed tasks keep running, so reaction-time misses, lapses and search time on this row include time the participant could not act. Recorded, not dropped; use it to define a sensitivity analysis.' },
