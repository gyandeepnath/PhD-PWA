...
export const ANALYSIS_CODEBOOK: AnalysisColumn[] = [
  ...
  { column: 'session_index', role: 'key', unit: 'index', missing: 'never empty',
    description: "This sitting's number for this participant (1 or 2)." },
  ...
  { column: 'illumination_c', role: 'coding', unit: '-0.5|+0.5', missing: 'unknown level',
    description: 'Sum-to-zero coding, dim = -0.5, moderate = +0.5. Same reasoning as polarity_c.' },
  { column: 'position_c', role: 'coding', unit: 'positions', missing: 'never empty',
    description: 'session_position centred on the middle of the sitting. Centring removes the correlation between the position term and the intercept, so the intercept remains interpretable as the average condition.' },

  // ---------------------------------------------------------------- order and position
  { column: 'session_position', role: 'covariate', unit: '1-10', missing: 'never empty',
  ...
  { column: 'global_position', role: 'covariate', unit: '1-20', missing: 'never empty',
    description: 'Order across BOTH sittings, continuing rather than restarting at the second visit. Use this for cumulative fatigue; use session_position for within-sitting fatigue. They answer different questions.' },
  ...
  { column: 'comprehension_correct', role: 'secondary', unit: 'count', missing: 'not completed',
    description: 'Correct items out of comprehension_items. Model binomially against that denominator rather than as a proportion.' },
  { column: 'comprehension_items', role: 'secondary', unit: 'count', missing: 'never empty',
    description: 'Items administered per passage — the denominator for comprehension_correct.' },
  ...
  { column: 'd_prime', role: 'secondary', unit: 'z', missing: 'block not completed',
  ...
  { column: 'cvsq_baseline_total', role: 'covariate', unit: 'score', missing: 'baseline not completed',
    description: 'Habitual computer-vision-syndrome score, taken before the sitting. A PARTICIPANT-LEVEL TRAIT: it is repeated across this participant\'s rows and must not be treated as a repeated measurement.' },
  { column: 'caffeine_today', role: 'covariate', unit: 'boolean', missing: 'not asked',
    description: 'Caffeine on the day of the sitting. Session-level, repeated across the rows of one sitting.' },
  { column: 'hours_since_sleep', role: 'covariate', unit: 'hours', missing: 'not asked',
    description: 'Hours awake at the start of the sitting. Session-level.' },

  // ---------------------------------------------------------------- session covariates
  { column: 'ambient_lux_measured', role: 'qc', unit: 'lux', missing: 'not measured',
    description: 'Measured illuminance at the eye at session start — the MANIPULATION CHECK for the illumination factor. Verify this separates the two levels before interpreting any illumination effect.' },
  { column: 'lux_all_in_range', role: 'qc', unit: 'boolean', missing: 'no readings logged',
    description: 'Whether every logged reading fell inside the accepted band for the assigned level. False marks a protocol deviation on the whole-plot factor.' },
  ...
  { column: 'fps_adequate_for_ratio', role: 'qc', unit: 'boolean', missing: 'camera not running',
    description: 'Whether the frame rate supports the primary outcome. Below the floor the sampled minimum EAR is biased UPWARD, so incomplete_blink_ratio is inflated — a directional bias, not symmetric noise. Rows are flagged and never dropped, because frame rate covaries with ambient illumination, which is an independent variable: dropping them would delete data non-randomly with respect to a factor.' },
