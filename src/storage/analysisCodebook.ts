...
export const ANALYSIS_CODEBOOK: AnalysisColumn[] = [
  ...
  { column: 'cvsq_baseline_total', role: 'covariate', unit: 'score', missing: 'baseline not completed',
    description: 'Habitual-frame CVS-Q taken at the start of THIS SITTING. SESSION-LEVEL, not a participant trait: a participant has one per sitting and they differ. Because sitting maps one-to-one onto illumination_block, within-participant drift here is perfectly aligned with the whole-plot contrast, and sitting 2\u2019s value is measured after sitting 1\u2019s ninety-minute exposure — so entering it as a covariate on the illumination effect absorbs part of the effect being estimated. State a reason if you use it that way.' },
  { column: 'caffeine_today', role: 'covariate', unit: 'boolean', missing: 'not asked',
    description: 'Caffeine on the day of THIS sitting, read from the session record. Genuinely session-level: it differs between a participant\u2019s two sittings, which is why it is asked each time. The participant record holds a sticky first-sitting copy that must NOT be used here.' },
