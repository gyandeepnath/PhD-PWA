...
function makePartialBundle(rng: Rng, nConds: number): SessionBundle {
  ...
  return {
    ...
    eyeMetrics: conditions.filter(has).map((c) => ({
      ...
      fps_adequate_for_tiers: false, fps_adequate_for_ratio: false, observed_duration_ms: 178000, ear_sample_count: 5340, open_ear_measured: rng() < 0.5 ? null : rng(), blink_rate: rng() * 30, blink_rate_full: rng() * 30, incomplete_blink_ratio: rng(),
      ...
      blink_count_micro: 0, blink_count_incomplete: 0, ear_baseline: null, ear_threshold_used: null, ear_complete_threshold: null,
