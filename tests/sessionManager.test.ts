...
function makeSession(id: string, over: Partial<SessionRecord> = {}): SessionRecord {
  return {
    ...
    deleted_at: null, display_label: null, ambient_lux: 350, screen_white_luminance_cd_m2: null,
...
describe('session manager admin', () => {
  ...
  it('purge cascades to all child records', async () => {
    ...
    await put('rt_summaries', { condition_id: 'c1', session_id: 'A', total_trials: 4, signal_trials: 2, hits: 2, false_alarms: 0, misses: 0, correct_rejections: 2, hit_rate: 1, false_alarm_rate: 0, mean_rt_hits_ms: 400, median_rt_hits_ms: 400, rt_sd_ms: 10, mean_rt_congruent_ms: 400, mean_rt_incongruent_ms: 400, flanker_congruency_effect_ms: 0, d_prime: 2, d_prime_se: 0.4, d_prime_unstable: true });
