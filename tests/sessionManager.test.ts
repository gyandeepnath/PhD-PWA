...
function makeSession(id: string, over: Partial<SessionRecord> = {}): SessionRecord {
  return {
    ...
    consent_time: 1, media_consent: { camera_metrics: true, setup_photos: false, annotation_video: false, granted_at: null }, provenance: prov, device_type: 'X', browser: 'Y', screen_resolution: '1x1', stimulus_scale: 1, layout_viewport: '1194x834',
...
describe('session manager admin', () => {
  ...
  it('purge cascades to all child records', async () => {
    await put('sessions', makeSession('A'));
    await put('participants', { participant_id: 'P-A', enrolment_number: 1, age: 30, gender: 'x', daily_screen_hours: 6, device_familiarity: 'high', lighting_habit: 'moderate', correction_type: 'none', cvd_status: 'normal', cvd_screen_correct: null, cvd_screen_total: null, cvd_clinical: 'normal' as const, caffeine_today: null, hours_since_sleep: null, eligible: true, exclusion_reason: null, baseline_fatigue: 0, session_id: 'A' });
